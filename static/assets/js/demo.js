...
$(document).ready(function () {
    // Function to get the message based on the time of day
    ...
    function showNotification() {
        const message = getTimeBasedMessage(); // Get the message based on the current time

        $.notify({
            icon: 'icon-bell', // Notification icon
            title: 'Gmobile', // Title of the notification
            message: message, // Message based on time
        }, {
            type: 'secondary', // Notification type (style)
            placement: {
                from: "bottom", // Position: bottom
                align: "right"  // Alignment: right
            },
            time: 3000, // Notification display duration (3 seconds)
        });
    }

    // Show the notification immediately when the page loads
    showNotification();

    // Optionally, update the notification every hour
    setInterval(showNotification, 3600000); // Refresh the message every 1 hour (3600000 ms)
});
